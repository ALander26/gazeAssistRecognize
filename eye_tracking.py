...
def main():
	...
	cap = cv2.VideoCapture('data/camera1.avi')
	while(cap.isOpened()):
		ret, frame = cap.read()

		gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)

		pupilCenter = gazeObject.GetPupilBoundaryPoints(gray)
		if pupilCenter is None:
			print "Blink!!"
		else:		
			LED_centroids = gazeObject.FindLEDCentroids(gray)
			if LED_centroids is None:
				print "fail to find LED_centroids"
			else:
				im2 = cv2.circle(frame, tuple(LED_centroids[0]), 10, (0,0,255), 2)
				im2 = cv2.circle(frame, tuple(LED_centroids[1]), 10, (0,0,255), 2)
			im2 = cv2.circle(frame, tuple(pupilCenter), 10, (0,0,255),2)
